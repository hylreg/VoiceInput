...
- **双击 Ctrl 卡键修复（2026-08-19）**：
  - 根因（已在 Xvfb + xev 复现并扫频证实）：监听循环在第二次按下的**按下沿**触发，主循环随即经 xdotool `--clearmodifiers` 注入文本（开始录音的 ● 提示符粘贴、停录后的退格）。xdotool clearmodifiers 的流程是「假松开修饰键 → 注入 → 假按下还原修饰键」，与用户随后物理松开修饰键存在竞态：物理松开若落在假松开与还原之间，还原会把 Ctrl 重新按下且此后无释放事件 → Ctrl 被系统永久视为按住（后续所有按键事件带 0x4 修饰位，再按一次 Ctrl 松开才恢复）。是否卡住取决于第二击按住时长与注入窗口的相对位置（● 路径含固定 40ms sleep，注入落在按下沿后约 50-70ms），故表现为概率性。
  - 修复：抽出 `DoublePressDetector` 边沿检测状态机，触发沿改为第二次按下的**释放沿**（与鼠标双击「第二次松开才触发」语义一致）——任何注入开始时修饰键均已物理松开，竞态窗口不复存在；800ms 冷却与 300ms 双击窗口语义不变。
  - 新增 6 个状态机单测（释放沿触发/单击不触发/窗口过期/按住等待松开/冷却抑制/冷却后新双击）；`cargo build/test --workspace` 与 `--features ibus` 全绿（21+4+2）；clippy 组件未安装（同 Task 10 记录）。
  - 已知残余：释放沿触发后到注入完成前（约几十 ms）用户若再次按下 Ctrl，理论上仍可能与 clearmodifiers 竞态，但概率极低（修复前是每次触发都存在的窗口，修复后需恰好在注入瞬间重新按下）；如需彻底消除可后续改为注入前显式 keyup 且不还原。
